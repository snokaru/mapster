...
namespace Mapster
{
    ...
    public class PrimitiveGroup : IEnumerable<IElement>
    {
        ...
        internal Func<int, IElement> ElementGetter { get; }

        public ElementType ContainedType { get; }

        public int Count { get; }

        public PrimitiveGroup(PrimitiveBlock primitiveBlock, OSMPBF.PrimitiveGroup? osmPrimitiveGroup = null, StringTable? stringTable = null)
        {
        ...
        private IElement GetUnknown(int index)
        {
            return new Elements.Unknown();
        }

        private IElement GetNode(int index)
        {
            return new Elements.SimpleNode(_osmPrimitiveGroup.Nodes[index], _primitiveBlock);
        }

        private IElement GetWay(int index)
        {
            return new Elements.Way(_osmPrimitiveGroup.Ways[index]);
        }

        private IElement GetRelation(int index)
        {
            return new Elements.Relation();
        }

        private IElement GetChangeSet(int index)
        {
            return new Elements.ChangeSet();
        }

        private IElement GetNextDenseNode(int index)
        {
    ...
    public class ElementEnumerator : IEnumerator<IElement>
    {
        ...
        private IElement _currentElement = new Elements.Unknown();
        private PrimitiveGroup _primitiveGroup;
        private int _elementCount = 0;
        private int _currentIndex = 0;

        public IElement Current => _currentElement;
